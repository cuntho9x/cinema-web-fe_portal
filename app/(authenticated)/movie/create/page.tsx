        ...
        <div className="movie-create-container">
          ...
          <form className="movie-create-form">
            <div className="movie-create-form-col">
              ...
              <input className="movie-create-input" placeholder="Nhập tên phim" />
              ...
              <textarea className="movie-create-input" placeholder="Nhập mô tả" rows={3} />
              ...
              <input className="movie-create-input" placeholder="Nhập tên đạo diễn " />

              <label className="movie-create-label">Diễn viên</label>
              <textarea className="movie-create-input" placeholder="Nhập tên diễn viên " rows={2} />
              </div>
              <div className="movie-create-form-col">
                <label className="movie-create-label required">Hình thức chiếu</label>
                <select className="movie-create-input">
                  <option>Chọn hình thức chiếu</option>
                  {graphicsOptions.map(g => <option key={g}>{g}</option>)}
                </select>
                <label className="movie-create-label required">Hình thức dịch</label>
                <select className="movie-create-input">
                  <option>Chọn hình thức dịch</option>
                  {translationOptions.map(g => <option key={g}>{g}</option>)}
                </select>
              <label className="movie-create-label required">Độ tuổi xem phim</label>
              <select className="movie-create-input"><option>chọn độ tuổi</option>{ageOptions.map(a => <option key={a}>{a}</option>)}</select>
              ...
              <input className="movie-create-input" placeholder="Nhập năm phát hành" />
              <label className="movie-create-label required">Thời lượng phim (phút)</label>
              <input className="movie-create-input" placeholder="Nhập độ dài phim" />
              ...
              <select className="movie-create-input"><option>Chọn quốc gia</option>{countryOptions.map(c => <option key={c}>{c}</option>)}</select>
            </div>
          </form>
        </div>
      </main>
